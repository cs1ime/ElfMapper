...
std::vector<uint64_t> elf_parse_initsect(u8*elf_file,u8* mapped)
{
	std::vector<uint64_t> result;
	
	auto addr = elf_get_dynseg_vaddr(elf_file,DT_INIT_ARRAY) + mapped;
	auto size = elf_get_dynseg_val(elf_file,DT_INIT_ARRAYSZ);
	...
	{
		auto funcaddr = *(uint64_t*)(addr+i*8);
		p1x(funcaddr);
		result.emplace_back(funcaddr);
	}
